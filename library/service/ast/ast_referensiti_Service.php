  ...
  public function getRuangRapatList($pageNumber, $itemPerPage) 
{
	...
	try 
	{
		...
		else
		{
			...
			q_gedung_luas,c_gedung_fungsi,i_orgb_tgjwbgedung
			FROM e_ast_ruangan_0_tr 
			where c_gedung_fungsi = ? ORDER BY i_ruang limit $xLimit offset $xOffset",$where);
	...
	public function getRuangList() {
	   ...
	   try {
	     $db->setFetchMode(Zend_Db::FETCH_OBJ); 
	     $result = $db->fetchAll('SELECT i_ruang,i_ruang_lokasi,n_gedung,c_gedung_fungsi
...
{	
	...
	public function getDataPegawai($prmRapatEdit) {
	   ...
	   try {
	     $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 $result = $db->fetchAll('SELECT a.i_peg_nip,n_peg,b.n_jabatan,i_orgb 
									FROM e_sdm_pegawai_0_tm A, e_sdm_jabatan_0_tm B 
									where a.i_peg_nip = b.i_peg_nip
									and a.i_peg_nip = ? ',$prmRapatEdit);
	...
	public function getRuangPindahList($unitkr) {
	   ...
	   try {
	     ...
	     $result = $db->fetchAll('SELECT i_ruang,i_ruang_lokasi,n_gedung,q_gedung_lantai,q_gedung_kapasitas,
		                                 q_gedung_luas,c_gedung_fungsi,i_orgb_tgjwbgedung
		                          FROM e_ast_ruangan_0_tr where c_gedung_fungsi = ? and i_orgb_tgjwbgedung =?
   ...
   public function getUnitKerja() {
	...
	public function getRuangPindahListbynip($pageNumber,$itemPerPage,$nip) {
	   ...
	   try {
		 ...
		 if(($pageNumber==0) && ($itemPerPage==0))
		 {
			$hasilAkhir = $db->fetchOne("select count(*) FROM e_ast_ruangan_0_tr where i_peg_nik = ? ",$nip);
		 }
		 else
		 {
			 ...
			 $result = $db->fetchAll("SELECT i_ruang,i_ruang_lokasi,n_gedung,q_gedung_lantai,q_gedung_kapasitas,
		                                 q_gedung_luas,c_gedung_fungsi,i_orgb_tgjwbgedung
										FROM e_ast_ruangan_0_tr where i_peg_nik = ?
										ORDER BY i_ruang
...
//======================== tambahan aceng 10 Desember ===============================
	...
	//======================================================
     ...
     // Ina : 16-04-2008 : Awal
     public function getListPenanggungJawabLama($pageNumber,$itemPerPage,$kdunit) {
	...
	// Ina : 16-04-2008 : Awal
    public function getListPenanggungJawabBaru($pageNumber,$itemPerPage,$nmPegawai) {
		$nmPegawai = strtoupper($nmPegawai);
		$npeg = '%'.$nmPegawai.'%';
