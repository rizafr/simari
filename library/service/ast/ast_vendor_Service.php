...
class ast_vendor_Service {
	...
	public function getVendorList_Bc($no_rekanan) {
	   ...
	   try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 $result = $db->fetchAll('SELECT i_rekanan, n_rekanan, a_rekanan,n_rekanan_agendistr, i_rekanan_telp, 
									n_rekanan_kontak, n_vendor_ctgr, i_rekanan_ref, a_prsh_jalan,i_rekanan_email
									FROM e_ast_vendor_0_tr where i_rekanan=?', $no_rekanan); 
         $jmlResult = count($result);
		 ...
		 for ($j = 0; $j < $jmlResult; $j++) {
           ...
           $hasilAkhir[$j] = array("i_rekanan"           		=>(string)$result[$j]->i_rekanan,
								   ...
								   "i_rekanan_email"			=>(string)$result[$j]->i_rekanan_email,
         ...
         if(($pageNumber==0) && ($itemPerPage==0))
			...
			{	
				...
				$result = $db->fetchAll("SELECT i_rekanan, n_rekanan, a_rekanan,n_rekanan_agendistr, i_rekanan_telp, n_rekanan_kontak,
										n_vendor_ctgr,i_rekanan_ref,a_prsh_jalan,i_rekanan_email 
										FROM e_ast_vendor_0_tr order by i_rekanan
										limit $xLimit offset $xOffset"); 
		         $jmlResult = count($result);
