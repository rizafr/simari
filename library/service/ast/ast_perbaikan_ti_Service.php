...
class ast_perbaikan_ti_Service {
		...
		$where[] = "N";
			...
			{ */
				...
				$setuju = $db->fetchAll("Select * from e_ast_ajuanbaikti_item_tm A, e_ast_ajuanbaikti_0_tm B
								...
								and a.c_status_perbaikan = ? and (a.c_setuju_sparepart is null	or a.c_setuju_sparepart ='')
								",$where)	;
	...
	public function getNoInventarisPCList_Ls($pageNumber,$itemPerPage,$kdunit, $nmBarang) {
	...
	$registry = Zend_Registry::getInstance();
	   ...
	   try {
	    $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 if(($pageNumber==0) && ($itemPerPage==0))
		 { 
			$hasilAkhir = $db->fetchOne("Select  A.d_anggaran, A.c_barang, A.i_aset, A.d_perolehan,  B.ur_sskel, 
										A.i_komputer_macaddress
										FROM  E_AST_KOMPUTER_0_TR A , E_AST_SSKEL_0_TR B
										Where A.C_Unit_Kerja = ?
										And B.kd_gol = substr(A.c_barang,1,1)
		 ...
		 else
		 {
			 ...
			 $xOffset=($pageNumber-1)*$itemPerPage;	
		      ...
		      $result = $db->fetchAll("Select  A.d_anggaran, A.c_barang, A.i_aset, A.d_perolehan,  B.ur_sskel, 
										...
										And B.kd_bid = '12'
										and b.ur_sskel like ?
										order by a.thn_ang,a.kd_brg,no_aset 
										limit $xLimit offset $xOffset ",$where);
	...
	//
	public function getNoInventarisPCList($pageNumber,$itemPerPage,$kdunit,$nmBarang) {
	...
	//
	public function getNoInventarisPCListByPeriode($pageNumber,$itemPerPage,$kdunit,$nmBarang,$tglAwal,$tglAkhir) {
	...
	$registry = Zend_Registry::getInstance();
	   ...
	   try {
	    $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 if(($pageNumber==0) && ($itemPerPage==0))
		 { 
			$result = $db->fetchOne("Select  A.d_anggaran, A.c_barang, to_char(A.i_aset,'09999') as i_aset , A.d_perolehan,  B.ur_sskel, 
										A.i_komputer_macaddress
										FROM  E_AST_KOMPUTER_0_TR A , E_AST_SSKEL_0_TR B
										Where A.C_Unit_Kerja = ?
										And B.kd_gol = substr(A.c_barang,1,1)
										...
										And B.kd_skel = substr(A.c_barang,6,2)
										and upper(b.ur_sskel) like ?	
										and A.d_perolehan  between ? and ?",$where);	
