...
class ast_penerimaan_atkdetail_Service {
	...
	public function updateTerimaAtkM(array $data) {
	   ...
	   try {
	     ...
	     $atk_dtl_parm = array("i_atk_kwtbeli"  	    =>$data['noKwitansi'],
							   "i_pgdaan_beritaacara"  	=>$data['NoBeritaAcr'],
						       "i_entry"       		    =>$data['nuser'],
						       "d_entry"       		    =>date("Y-m-d"));
	...
	public function queryVerifikasiAtkD($noajuan) {
	   ...
	   try {
		 ...
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 $result = $db->fetchAll('SELECT b.c_atk_ctgr,b.c_atk,n_atk,q_atk_terima,b.v_atk_hrgsatuan,q_atk_realterima,q_atk_stock
		                         FROM e_ast_barang_atk_tr a, e_ast_terima_itematk_tm b,e_ast_terima_atk_tm c
	   ...
	   $db = $registry->get('db');
	    try {
		 ...
		 $where[] = "c_atk_ctgr    =  '".trim($data['KatBarang'])."'";		 
	        ...
	        $atk_dtl_parm = array("q_atk_stock"  		        =>$data['stock'] + $data['Jml'],
							       "i_entry"       		        =>$data['nuser'],
							       "d_entry"       		        =>date("Y-m-d"));
	...
	public function updateStock2(array $data) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	    try {
	     $db->beginTransaction();
	...
	//=======================service ini utk penerimaan tanpa pengajuan/atk langsung ========================add 21 feb 08=((not get q_setuju beli atk)==============================
	public function queryPenerimaanAtkDt($nopeng) { 
	   ...
	   try {
		 ...
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 $result = $db->fetchAll('SELECT b.c_atk_ctgr,d.n_atk_ctgr,b.c_atk,n_atk,n_atk_satuan,n_atk_merek,n_atk_tipe,
									q_atk_terima,q_atk_realterima,b.v_atk_hrgsatuan,i_atk_kwtbeli,d_atk_beli,q_atk_stock ,c_matauang, c.i_pgdaan_beritaacara
									FROM e_ast_barang_atk_tr a, e_ast_terima_itematk_tm b,e_ast_terima_atk_tm c,
									e_ast_kategori_atk_tr d
									where b.i_atk_terima = ? and a.c_atk = b.c_atk and b.i_atk_terima = c.i_atk_terima 
									and b.c_atk_ctgr=d.c_atk_ctgr 
									ORDER BY c_atk' ,$nopeng);
         $jmlResult = count($result);
		 for ($j = 0; $j < $jmlResult; $j++) {
	...
	public function queryPenerimaanAtkD1t($nopeng) {
	   ...
	   try {
		 ...
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
		 $result = $db->fetchAll('SELECT b.c_atk_ctgr,d.n_atk_ctgr,b.c_atk,n_atk,n_atk_satuan,n_atk_merek,n_atk_tipe,
									q_atk_terima,q_atk_realterima,b.v_atk_hrgsatuan,i_atk_kwtbeli,d_atk_beli,q_atk_stock,c_matauang
									FROM e_ast_barang_atk_tr a, e_ast_terima_itematk_tm b,e_ast_terima_atk_tm c,
									e_ast_kategori_atk_tr d,,e_ast_ajuanbeli_itematk_tm e
									where b.i_atk_terima = ? and a.c_atk = b.c_atk and b.i_atk_terima = c.i_atk_terima 
									and b.c_atk_ctgr=d.c_atk_ctgr 
