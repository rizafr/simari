...
class pengajuancuti_Service {
    ...
    public function getPegawaiList($cari, $currentPage, $numToDisplay,$cJeniscuti=null) {
        ...
        try {
            if (($currentPage == 0) && ($numToDisplay == 0)) {
                ...
                for ($j = 0; $j < $jmlResult; $j++) {
                    ...
                    // hitung jml maksimal dari jenis cutinya
                    $sqlMaxLamaCuti = $db->fetchOne("SELECT max_lama_cuti FROM sdm.tr_cuti_pegawai WHERE c_cuti='$cJeniscuti'");
                    // hitung keseluruhan jumlah cuti yang diambil dilihat dari nip nya
                    $sqlItungSisaCutiPeg = "SELECT SUM(q_lama_cuti) FROM sdm.tm_ajuan_cuti WHERE trim(i_peg_nip)='".trim($i_peg_nip)."' AND c_jenis_cuti='$cJeniscuti' AND to_char(d_cuti_mulai, 'yyyy')=to_char(now(), 'yyyy')";
                    $fetchItungSisaCutiPeg = $db->fetchOne($sqlItungSisaCutiPeg);
                    // hitung cuti bersama dalam 1 tahun
                    $sqlCountJmlCutiBersama = $db->fetchOne("SELECT COUNT(d_tgl_libur) FROM sdm.e_sdm_abs_harilibur_tm WHERE to_char(d_tgl_libur, 'yyyy')=to_char(now(), 'yyyy') ");
                    $kondisiJmlCti = ($cJeniscuti == "T") ? $sqlCountJmlCutiBersama : 0;
                    $i_sisa_cuti = ($sqlMaxLamaCuti - $fetchItungSisaCutiPeg) - $kondisiJmlCti;
                    ...
                    if ($neselon5) {
                        ...
                        $eselon = $eselon . $neselon5;
                    }
                    if ($neselon4) {
                        $eselon = $eselon . $neselon4 . ", ";
                    } else {
                        $eselon = $eselon . $neselon4;
                    }
                    if ($neselon3) {
                        $eselon = $eselon . $neselon3 . ", ";
                    } else {
                        $eselon = $eselon . $neselon3;
                    }
                    if ($neselon2) {
                        $eselon = $eselon . $neselon2 . ", ";
                    } else {
                        $eselon = $eselon . $neselon2;
                    }
                    if ($neselon1) {
                        $eselon = $eselon . $neselon1;
                    } else {
                        $eselon = $eselon . $neselon1;
                    }
...
//            print_r($data);
    ...
    public function maintainDataCpns(array $data, $par) {
        ...
        try {
            ...
            if ($par == 'insert') {
                $db->insert('sdm.tm_ajuan_cuti', $maintain_data);
            }
            if ($par == 'update') {
                $db->update('sdm.tm_ajuan_cuti', $maintain_data, "i_peg_nip = '" . trim($data['i_peg_nip']) . "' ");
            }
