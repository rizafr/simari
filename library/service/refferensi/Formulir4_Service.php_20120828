...
class formulir4_Service {
	...
	public function getformulir4($tahun)  
	{
	...
	public function getJumformulir4($tahun)  
	{
			$registry = Zend_Registry::getInstance();
			$db = $registry->get('db');
			try 
	...
	public function getgol()  
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					...
					$result = $db->fetchAll("select trim(n_peg_golongan) as n_peg_golongan, trim(n_peg_pangkat) as n_peg_pangkat, count(trim(n_peg_golongan)) as jumlah from 
								sdm.tr_golongan_pangkat a right join sdm.tm_pegawai b on(b.c_golongan = a.c_peg_golongan) 
								where c_peg_tipegolongan='3' group by n_peg_golongan, n_peg_pangkat order by n_peg_golongan DESC");
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
					{
	...
	public function tambahformulir4(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
	...
	public function detailFormulir4($masukan) 
	{
			$registry = Zend_Registry::getInstance();
			$db = $registry->get('db');
			try 
	...
	public function ubahformulir4(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		...
		try {
			...
			$paramInput = array("pangkat"	=> $pangkat,
					...
					"d_entry"	=> date('Y-m-d H:i:s'));
					//var_dump($paramInput);
			$db->update('sdm.tm_formasiformulir4',$paramInput, $where);
			$db->commit();
