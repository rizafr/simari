...
class formulir2_Service {
	...
	public function getformulir2($tahun)  
	{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					...
					for ($j = 0; $j < $jmlResult; $j++) 
					{
						$data[$j] = array("id"=>(string)$result[$j]->id,
									...
									"formasi_lajur"=>(string)$result[$j]->formasi_lajur,
	...
	public function tambahformulir2(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		...
		try {
			...
			if($dataMasukan['formasi_lajur'] == ""){
			$formasi_lajur = 0;
			}else{
			$formasi_lajur = $dataMasukan['formasi_lajur'];
			}
	...
	public function detailFormulir2($masukan) 
	{
			$registry = Zend_Registry::getInstance();
			$db = $registry->get('db');
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
				$result = $db->fetchRow("select gol_ruang_gaji, bezetting, thn_skrg, thn_sblm, knaikn_pngkt, stlh_naik_pgkt, 
							pndah_dr_instasi_lain, pndah_ke_instasi_lain, pengangkt_pgwai_br, formasi_lajur, ket
							from sdm.tm_formasiformulir2 
							where id  = '".$masukan['id']."'");
				...
				$data = array("gol_ruang_gaji"=>(string)$result->gol_ruang_gaji,
					      ...
					      "formasi_lajur"=>(string)$result->formasi_lajur,
	...
	public function ubahformulir2(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		...
		try {
			...
			$paramInput = array("gol_ruang_gaji"	=> $gol_ruang_gaji,
					...
					"d_entry"	=> date('Y-m-d H:i:s'));
					//var_dump($paramInput);
			$db->update('sdm.tm_formasiformulir2',$paramInput, $where);
			$db->commit();
