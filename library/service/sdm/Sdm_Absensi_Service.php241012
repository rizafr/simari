...
//bamris
    ...
    public function getAbsnMasukPegawai($terminal, $nip, $tgl) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');
        $sql = "select min(d_peg_jam) from absensi.e_sdm_absensi_mesin_ts where c_terminal='" . $terminal . "' and i_peg_nip='" . $nip . "' and to_char(d_peg_absensi,'yyyymmdd')='" . $tgl . "'";
//echo $sql."<br>";
        try {
            $db->setFetchMode(Zend_Db::FETCH_OBJ);
            $result = $db->fetchOne($sql);
...
//author bamris
    ...
    public function getAbsensiListPP(array $data, $pageNumber, $itemPerPage) {
        ...
        try {
            ...
            if (substr($idORG, 0, 2) == "SK")
                $idORG = substr($idORG, 0, 2);
            $idOrder = $data['ord'];
            if (($pageNumber == 0) && ($itemPerPage == 0)) {
    ...
    public function getJamHariKerja($tanggal) {
        ...
        try {
            ...
            $hasilAkhir = array(
                "d_tgl_kerja" => (string) $rsSelect[0]->d_tgl_kerja,
    ...
    public function insHariLibur(array $data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');
        try {
            $db->beginTransaction();
            $harilibur = array(
                "d_tgl_libur" => $data['d_tgl_libur'],
                "i_keterangan" => $data['i_keterangan'],
                "i_entry" => $data['i_entry'],
                "d_entry" => $data['d_entry']
            );
            $db->insert('sdm.e_sdm_abs_harilibur_tm', $harilibur);
            $db->commit();
            return 'sukses';
        } catch (Exception $e) {
            $db->rollBack();
...
//                    "d_jamistrht_selesai from sdm.e_sdm_abs_jamharikerja_tr where c_hari=" . $hari);
            ...
            $rsSelecta = $db->fetchAll("select d_jamkerja_mulai,d_jamkerja_selesai,d_jamistrht_mulai,d_jamistrht_selesai 
            from sdm.e_sdm_abs_harikerja_tm where to_char(d_tgl_kerja,'dd/mm/yyyy')='".$hari."' ");
            $output = array(
                "d_jamkerja_mulai" => $rsSelecta[0]->d_jamkerja_mulai,
                "d_jamkerja_selesai" => $rsSelecta[0]->d_jamkerja_selesai,
                "d_jamistrht_mulai" => $rsSelecta[0]->d_jamistrht_mulai,
                "d_jamistrht_selesai" => $rsSelecta[0]->d_jamistrht_selesai
            );
            return $output;
