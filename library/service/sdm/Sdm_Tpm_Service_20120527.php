 ...
 //==================================================================================================================================
 	public function getTpmPegawai($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchall("select distinct(tpm_id) as tpm_id,a.id_list,usulan_id,peg_nip,tvjabatan_kd,tvinstansi_kd,tpm_keterangan,
								tpm_status,to_char(tpm_tgl,'dd-mm-yyyy') as tpm_tgl,tvinstansi_kd_l,ljabatangol_nm,tpm_jekel,tpm_tgllahir,
								tpm_namalengkap,lap_gol_tmt,lap_gol_nm,lap_jab_l_nm,lap_jab_l_kelas,lap_jab_l_instan,
								lap_jab_b_nm,lap_jab_b_kelas,lap_jab_b_instan,tpm_status_1,tpm_status_2,tpm_status_3,nama_list,nourut,tpm_stts
								from sdm.tm_pegawai_tpm a, sdm.tm_list_tpm b where a.id_list=b.id_list $cari  order by id_list desc");		
					$jmlresult = count($result);
					for ($j = 0; $j < $jmlresult; $j++) 
					{ 
 ...
 //==================================================================================================================================
  	public function getListTpm($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchall("select distinct(a.usulan_id) as usulan_id ,a.usulan_nomor,
