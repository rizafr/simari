...
class Sdm_Pangkat_Service {
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
		...
		if ($par=='update'){$db->update('sdm.tm_golongan_pangkat',$maintain_data, "i_peg_nip = '".trim($data['i_peg_nip'])."'  and c_golongan = '".trim($data['c_golongan'])."'  and to_char(d_tmt_golongan,'yyyy-mm-dd') = '".trim($data['d_tmt_golongan'])."' and c_jenis_naik = '".trim($data['c_jenis_naik'])."'");}	 
		if ($par=='delete'){$db->delete('sdm.tm_golongan_pangkat', "i_peg_nip = '".trim($data['i_peg_nip'])."'  and c_golongan = '".trim($data['c_golongan'])."' and to_char(d_tmt_golongan,'yyyy-mm-dd') = '".trim($data['d_tmt_golongan'])."' and c_jenis_naik = '".trim($data['c_jenis_naik'])."'");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
