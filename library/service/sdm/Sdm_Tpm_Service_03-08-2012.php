 ...
 //==================================================================================================================================
 	public function getTpmPegawai($cari) 
	{
			...
			try 
			{
				...
				$sql = "select tpm_id,a.id_list,usulan_id,peg_nip,a.tvinstansi_kd,tpm_keterangan, tpm_status,tvinstansi_kd_l,ljabatangol_nm,tpm_jekel,tpm_tgllahir, tpm_namalengkap,lap_gol_tmt,lap_gol_nm,lap_jab_l_nm,lap_jab_l_kelas,lap_jab_l_instan, lap_jab_b_nm,lap_jab_b_kelas,lap_jab_b_instan,tpm_status_1,tpm_status_2,tpm_status_3,(select usulan_keterangan from sdm.tm_usulan_tpm where usulan_id=a.usulan_id) as usulan_keterangan,to_char(tpm_tgl,'dd-mm-yyyy') as tpm_tgl from sdm.tm_pegawai_tpm a where $cari  order by id_list desc";
					//echo $sql."<br>";
  	...
  	public function getListTpm2($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					...
					$sql = "select distinct(a.usulan_id) as usulan_id ,a.usulan_nomor,to_char(mod_date,'dd-mm-yyyy') as mod_date,(SELECT to_char(tpm_tgl,'dd-mm-yyyy') FROM sdm.tm_pegawai_tpm  where usulan_id =a.usulan_id group by tpm_tgl) as tpm_tgl, a.tvinstansi_kd,a.periode_text,a.periode_keterangan,a.usulan_keterangan as nama_list,a.usulan_keterangan	from sdm.tm_usulan_tpm a, sdm.tm_pegawai_tpm b  where  $cari";
					//echo $sql;
					$result = $db->fetchall($sql);	
