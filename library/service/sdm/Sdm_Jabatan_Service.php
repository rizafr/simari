...
class Sdm_Jabatan_Service {
    ...
    public function cekNamaUk($nama, $cesl) {
        ...
        if ($cesl == '01') {
            ...
            $data = str_replace("jenderal", "", $data);
            $data = str_replace("badan", "", $data);
            $data = str_replace("sekretariat ma", "", $data);
        }
        if ($cesl == '03') {
            $data = str_replace("direktorat", "", $data);
            $data = str_replace("biro", "", $data);
        ...
        if ($cesl == '05') {
            ...
            $data = str_replace("bagian", "", $data);
            $data = str_replace("bidang", "", $data);
            $data = str_replace("balai", "", $data);
        }
        if ($cesl == '04') {
            $data = str_replace("panitera", "", $data);
            $data = str_replace("muda", "", $data);
        }
        if ($cesl == '07') {
            $data = str_replace("direktorat", "", $data);
            $data = str_replace("sub", "", $data);
    ...
    public function getJabatanList($cari) {
        ...
        try {
            ...
            $result = $db->fetchAll("SELECT  id,i_peg_nip,c_eselon,to_char(d_tmt_eselon,'dd-mm-yyyy') as d_tmt_eselon,
										c_jabatan,n_jabatan_nokode,to_char(d_mulai_jabat,'yyyy-mm-dd') as d_mulai_jabat,
										...
										e_njabatan,j_jabatan,e_jabatanket,e_unit,e_instansi		
										FROM sdm.tm_jabatan where 1=1 $cari  order by d_mulai_jabat desc");
            ...
            for ($j = 0; $j < $jmlResult; $j++) {
                ...
                if ($c_lokasi_unitkerja == '1') {
                    ...
                    if ($c_eselon_v && $c_eselon != '15') {
                        $n_eselon_iv = $this->cekNamaUk($n_eselon_v, $c_esl_v);
                        if ($n_eselon_v) {
                            $nesl5 = "$c_esl_v $n_eselon_v pada";
                        }
                    } else {
                        if ($n_eselon_v) {
                            if ($c_eselon == '15') {
                                $nesl5 = " pada $n_eselon_v,";
                                $par = 'y';
                            } else {
                                $nesl5 = " $n_eselon_v,";
                            }
                    ...
                    if ($c_eselon_iv && (!$c_eselon_v || $c_eselon_v == '00') && $c_eselon != '15') {
                        $n_eselon_iv = $this->cekNamaUk($n_eselon_iv, $c_esl_iv);
                        if ($n_eselon_iv) {
                            $nesl4 = "$n_eselon_iv pada";
                        }
                    } else {
                        if ($n_eselon_iv) {
                            if ($c_eselon == '15') {
                                $nesl4 = " pada $n_eselon_iv,";
                                $par = 'y';
                            } else {
                                $nesl4 = " $n_eselon_iv,";
                            }
                    ...
                    if ($c_eselon_iii && (!$c_eselon_iv || $c_eselon_iv == '00') && (!$c_eselon_v || $c_eselon_v == '00') && $c_eselon != '15') {
                        $n_eselon_iii = $this->cekNamaUk($n_eselon_iii, $c_esl_iii);
                        if ($n_eselon_iii) {
                            $nesl3 = "$n_eselon_iii pada";
                        }
                    } else {
                        if ($n_eselon_iii) {
                            if ($c_eselon == '15' && $par == 'n') {
                                $nesl3 = " pada $n_eselon_iii,";
                                $par = 'y';
                            } else {
                                $nesl3 = " $n_eselon_iii,";
                            }
                    ...
                    if ($c_eselon_ii && (!$c_eselon_iii || $c_eselon_iii == '00') && (!$c_eselon_iv || $c_eselon_iv == '00') && (!$c_eselon_v || $c_eselon_v == '00') && $c_eselon != '15') {
                        $n_eselon_ii = $this->cekNamaUk($n_eselon_ii, $c_esl_ii);
                        if ($n_eselon_ii) {
                            $nesl2 = "$n_eselon_ii pada";
                        }
                    } else {
                        if ($n_eselon_ii) {
                            if ($c_eselon == '15' && $par == 'n') {
                                $nesl2 = " pada $n_eselon_ii,";
                                $par = 'y';
                            } else {
                                $nesl2 = " $n_eselon_ii,";
                            }
                    ...
                    if ($c_eselon_i && (!$c_eselon_ii || $c_eselon_ii == '00') && (!$c_eselon_iii || $c_eselon_iii == '00') && (!$c_eselon_iv || $c_eselon_iv == '00') && (!$c_eselon_v || $c_eselon_v == '00') && $c_eselon != '15') {
                        $n_eselon_i = $this->cekNamaUk($n_eselon_i, $c_esl_i);
                        if ($n_eselon_i) {
                            $nesl1 = "$n_eselon_i";
                        }
                    } else {
...
//echo "noer $e_eselon,$sqlv ";
	...
	if ($c_eselon == '13'){ //fungsional
		...
		$e_jabatan = $n_jabatan . ' '.$e_eselon; //($c_split > 1 ? $r_unitr[$c_split-1] :'').'' ;
                ...
                //if(trim($result[$j]->e_njabatan != '' )) $e_jabatan = trim($result[$j]->e_njabatan);
                //else if(trim($result[$j]->e_jabatanket != '' )) $e_jabatan = trim($result[$j]->e_jabatanket);
                if ($c_jabatan == '') {
                    if (trim($result[$j]->e_njabatan != ''))
                        $e_jabatan = trim($result[$j]->e_njabatan);
                    else if (trim($result[$j]->e_jabatanket != ''))
                        $e_jabatan = trim($result[$j]->e_jabatanket);
                }
                /////////////////////////////////////////////////////////////////////////////////////
                $data[$j] = array("id" => (string) $result[$j]->id,
                    ...
                    "e_jabatan" => $e_jabatan,
                    "e_jabatanket" => (string) $result[$j]->e_jabatanket,
                    "e_njabatan" => (string) $result[$j]->e_njabatan,
                    "e_unit" => (string) $result[$j]->e_unit,
                    "e_instansi" => (string) $result[$j]->e_instansi,
                    "n_jabatan" => $n_jabatan,
                    ...
                    "c_parent" => trim($result[$j]->c_parent),
                    "c_satker" => trim($result[$j]->c_satker),
                    "e_njabatan" => trim($result[$j]->e_njabatan),
                    "j_jabatan" => trim($result[$j]->j_jabatan),
    ...
    public function maintainData(array $data, $par) {
        ...
        try {
            ...
            $maintain_data = array(
                ...
                "e_njabatan" => $data['e_njabatan'],
                "j_jabatan" => $data['j_jabatan'],
                "i_entry" => $data['i_entry'],
                "d_entry" => date("Y-m-d")
            );

            if ($par == 'insert') {
                $db->insert('sdm.tm_jabatan', $maintain_data);
            }
            ...
            if ($par == 'update') {
                $db->update('sdm.tm_jabatan', $maintain_data, "id = '" . trim($data['id']) . "'");
            }
            if ($par == 'delete') {
                $db->delete('sdm.tm_jabatan', "id = '" . trim($data['id']) . "' ");
            }
    ...
    public function updateTmPegawai(array $data) {
        ...
        try {
            ...
            $db->beginTransaction();
		$q_tktfgs = $data['q_tktfgs'] ? $data['q_tktfgs'] : 0;
