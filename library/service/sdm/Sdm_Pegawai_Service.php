	...
	public function getListPegawai(){
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					$sql="SELECT i_peg_nip,i_peg_nrp,i_peg_nip_new, n_peg 
							FROM sdm.tm_pegawai where 
							c_golongan isnull or  c_golongan =''
								ORDER BY c_golongan ASC
								limit 1000 offset 0";
								//echo $sql;
...
public function cariSatker($unitkerja){
            $registry = Zend_Registry::getInstance();
            $db = $registry->get('db');
            try 
            {
	...
	public function updatepangkat($i_peg_nip,$c_golongan,$d_tmt_golongan){
			...
			try 
			{
				...
				$xLimit=$numToDisplay;
				$xOffset=($currentPage-1)*$numToDisplay;	
				if($d_tmt_golongan == '' ) $dt_tmt_golongan= 'null';
				else $dt_tmt_golongan= "'".$d_tmt_golongan."'";
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					$sql="update sdm.tm_pegawai set c_golongan='$c_golongan'
							, d_tmt_golongan= $dt_tmt_golongan where 
							i_peg_nip='$i_peg_nip' ";
								//echo $sql;
					$result = $db->fetchAll($sql);
...
public function getlastpangkat($i_peg_nip){
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					$sql="SELECT * 
							FROM sdm.tm_golongan_pangkat where 
							i_peg_nip='$i_peg_nip' and c_golongan != ''
								ORDER BY c_golongan ASC
								limit 1 offset 0";
								//echo $sql;
					$result = $db->fetchAll($sql);
 	...
 	public function getPegawaiList($cari,$currentPage, $numToDisplay,$orderby){
			...
			{	
				...
				$xOffset=($currentPage-1)*$numToDisplay;				
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					$sql="SELECT n_peg_gelardepan,n_peg_gelarblkg,i_peg_nip,i_peg_nrp,i_peg_nip_new, n_peg,c_peg_status,c_status_kepegawaian,c_eselon,to_char(d_tmt_eselon,'yyyy-mm-dd') as d_tmt_eselon,
	...
	$n_eselon_i="";		
	$c_jabatan="";
	if (trim($result[$j]->c_peg_status)!='2CP') // PNS
	{
	...
	else // CPNS
	{
		...
		if (!$c_eselon_iv){$c_eselon_iv=trim($result[$j]->c_eselon_ii_cpns);}
		if (!$c_eselon_iv){$c_eselon_iv=trim($result[$j]->c_eselon_iii_cpns);}
		if (!$c_eselon_iv){$c_eselon_iv=trim($result[$j]->c_eselon_iv_cpns);}
		if (!$c_eselon_v){$c_eselon_v=trim($result[$j]->c_eselon_v_cpns);}
		if (!$c_eselon){$c_eselon=trim($result[$j]->c_eselon_cpns);}
		if (!$c_jabatan){$c_jabatan=trim($result[$j]->c_jabatan_cpns);}
		if (!$c_golongan){$c_golongan=trim($result[$j]->c_gol_cpns);}
...
else
{
	...
	if (trim($c_lokasi_unitkerja)=='1'){
		if ($c_eselon_v  && $c_eselon_v != '00') {
			$sqlv = "SELECT n_unitkerja FROM sdm.tr_unitkerja WHERE c_lokasi_unitkerja='$c_lokasi_unitkerja' and c_eselon_i='$c_eselon_i' and c_eselon_ii='$c_eselon_ii' and c_eselon_iii='$c_eselon_iii' and c_eselon_iv='$c_eselon_iv' and c_eselon_v='$c_eselon_v' ";
			$e_eselon_v = trim($db->fetchOne($sqlv));
			$e_eselon = $e_eselon_v;
		}
		if ($c_eselon_iv  && $c_eselon_iv != '00') {
			$sqliv = "SELECT n_unitkerja FROM sdm.tr_unitkerja WHERE c_lokasi_unitkerja='$c_lokasi_unitkerja' and c_eselon_i='$c_eselon_i' and c_eselon_ii='$c_eselon_ii' and c_eselon_iii='$c_eselon_iii' and c_eselon_iv='$c_eselon_iv' and c_tkt_esl='4'";
			$e_eselon_iv = trim($db->fetchOne($sqliv));
			$e_eselon = $e_eselon_iv;
		}
	} else if (trim($c_lokasi_unitkerja)=='3'){
		if ($c_eselon_v  && $c_eselon_v != '00') {
			$sqlv = "SELECT n_unitkerja FROM sdm.tr_unitkerja WHERE c_lokasi_unitkerja='$c_lokasi_unitkerja' and c_eselon_i='$c_eselon_i' and c_eselon_ii='$c_eselon_ii' and c_eselon_iii='$c_eselon_iii' and c_eselon_iv='$c_eselon_iv' and c_eselon_v='$c_eselon_v' ";
			$e_eselon_v = trim($db->fetchOne($sqlv));
			$e_eselon = $e_eselon_v;
	...
	///////////////
	if (trim($c_lokasi_unitkerja)=='1' && $c_eselon_i != '' && $c_eselon_i != '00') {
	...
	if ($c_eselon == '13'){ //fungsional
		...
		else $e_jabatan = $n_jabatan . ' pada '. ($e_eselon_v ? " $e_eselon_v,":"") . ($e_eselon_iv ? " $e_eselon_iv,":""). ($e_eselon_iii ? " $e_eselon_iii":"") . ($e_eselon_iii && $e_eselon_ii ? ", ":"").($e_eselon_ii ? " $e_eselon_ii":"");
		
	} else if ($c_eselon =='14'){ // TTP
		if ($c_lokasi_unitkerja == '1') $e_jabatan = $n_jabatan . ' pada '. ($e_eselon_iv ? " $e_eselon_iv,":"") . ($e_eselon_iii ? " $e_eselon_iii,":"") . ($e_eselon_ii ? " $e_eselon_ii,":"") . ($e_eselon_i ? " $e_eselon_i":"");
		else   $e_jabatan = $n_jabatan . ' pada '. ($e_eselon_iv ? " $e_eselon_iv":"") .($e_eselon_iv && $e_eselon_iii ? ', ':''). ($e_eselon_iii ? " $e_eselon_iii":"");
	...
	public function getCutiList($cari,$currentPage, $numToDisplay,$orderby){
			...
			try 
			{
				if(($currentPage==0) && ($numToDisplay==0))
			{
				$data = $db->fetchOne("select count(a.*) from sdm.tm_ajuan_cuti a left join sdm.tm_pegawai b on a.i_peg_nip=b.i_peg_nip where 1=1  $cari");
			...
			{	
				...
				$xOffset=($currentPage-1)*$numToDisplay;				
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
					$sql="SELECT a.i_peg_nip,i_peg_nrp,i_peg_nip_new, n_peg,c_peg_status,c_status_kepegawaian,c_eselon,to_char(d_tmt_eselon,'yyyy-mm-dd') as d_tmt_eselon,
								c_jabatan,to_char(d_mulai_jabat,'dd-mm-yyyy') as d_mulai_jabat,c_peg_jeniskelamin,
								to_char(d_akhir_jabat,'dd-mm-yyyy') as d_akhir_jabat,c_golongan,d_tmt_golongan,v_gaji_pokok,
								c_lokasi_unitkerja,c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,c_gol_cpns,
								c_lokasi_unitkerja_cpns,c_eselon_cpns,e_file_photo,c_eselon_cpns,c_lokasi_unitkerja_cpns,c_eselon_i_cpns,c_satker_cpns,c_parent_cpns
								...
								c_satker,c_parent,c_child,c_lokasi_unitkerja_pns,c_eselon_i_pns,c_eselon_ii_pns,c_eselon_iii_pns,c_eselon_iv_pns,c_satker_pns,c_parent_pns,
								to_char(d_peg_lahir,'yyyy-mm-dd') as d_peg_lahir,
								c_jenis_cuti,to_char(d_cuti_mulai,'dd-mm-yyyy') as d_cuti_mulai,to_char(d_cuti_akhir,'dd-mm-yyyy') as d_cuti_akhir,
								q_lama_cuti,a_alamat_cuti,a_cuti_rt,a_cuti_rw,a_cuti_propinsi,a_cuti_kota,a_cuti_kodepos,q_cuti_telponrumah,q_cuti_telponhp,e_cuti_alasan,i_surat_cuti,i_sisa_cuti,i_peg_nip_atasan
								FROM sdm.tm_ajuan_cuti a left join sdm.tm_pegawai b on a.i_peg_nip=b.i_peg_nip where 1=1 $cari  
								$orderBy
								limit $xLimit offset $xOffset";
	...
	$n_status_kepegawaian="";
	$n_status_kepegawaian= $db->fetchOne("SELECT n_status_kepegawaian FROM sdm.tr_status_kepegawaian WHERE c_status_kepegawaian ='$c_status_kepegawaian'");
	$n_unitkerja = $db->fetchOne("SELECT n_unitkerja FROM sdm.tr_unitkerja WHERE c_lokasi_unitkerja='$c_lokasi_unitkerja' AND c_eselon_i='$c_eselon_i' AND c_eselon_ii='$c_eselon_ii' AND c_eselon_iii='$c_eselon_iii' AND c_eselon_iv='$c_eselon_iv' AND c_eselon_v='$c_eselon_v'");
...
else
{
	...
	if ($c_eselon == '13'){ //fungsional
		...
		if ($e_eselon_iii) list($awalan41,$awalan42,$r_unit4) = split(' ',$e_eselon_iv,3);
		$r_unit4x = '';
		if ($e_eselon_iv) list($awalan41,$awalan42,$r_unit4x) = split(' ',$e_eselon_iv,3);
		//$e_jabatan = $n_jabatan . ' '. $r_unit4. ($e_eselon_iii ? " pada $e_eselon_iii":"") . ($e_eselon_ii ? ", $e_eselon_ii":"") . ($e_eselon_i ? ", $e_eselon_i":"");
		if ($c_lokasi_unitkerja == '1')$e_jabatan = $n_jabatan . ' '. $r_unit4. ($e_eselon_iii ? " pada $e_eselon_iii":"") . ($e_eselon_ii ? ", $e_eselon_ii":"") . ($e_eselon_i ? ", $e_eselon_i":"");
		else $e_jabatan = $n_jabatan . ' '. $r_unit4x . ' pada ' . ($e_eselon_iii ? " $e_eselon_iii,":"") . ($e_eselon_ii ? " $e_eselon_ii,":"") . ($e_eselon_i ? " $e_eselon_i":"");	
	} else if ($c_eselon =='15'){ // staff
		//$r_units = '';
		//if ($e_eselon_iii) list($awalan41,$awalan42,$r_unit4) = split(' ',$e_eselon_iv,3);
		if ($c_lokasi_unitkerja == '1') $e_jabatan = $n_jabatan . ' pada '. ($e_eselon_iv ? " $e_eselon_iv,":"") . ($e_eselon_iii ? " $e_eselon_iii,":"") . ($e_eselon_ii ? " $e_eselon_ii,":"") . ($e_eselon_i ? " $e_eselon_i":"");
	...
	$unitkerjalengkap=$nesl5.$nesl4.$nesl3.$nesl2.$nesl1;
		...
		$data[$j] = array("i_peg_nip"=>(string)$result[$j]->i_peg_nip,
			...
			"i_surat_cuti"=>(string)$result[$j]->i_surat_cuti,
			"i_sisa_cuti"=>(string)$result[$j]->i_sisa_cuti,
			"i_peg_nip_atasan"=>(string)$result[$j]->i_peg_nip_atasan,
	...
	public function insertUploadExcel(array $data,$par){
            $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
	     $db->beginTransaction();
	     ...
	     $maintain_data = array(
            ...
            if($data['i_peg_nip'] != "") { 
                $t = array("i_peg_nip" => $data['i_peg_nip']);
                $maintain_data = array_merge($t,$maintain_data);
            } else {}
...
//                    print_r($maintain_data);
                    $db->update('sdm.tm_pegawai',$maintain_data, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");    
                }
//                exit;
			
		if(trim($data['i_peg_nip']) != '' && trim($data['i_peg_nip']) != trim($data['i_peg_nipb'])){
			//update table lain
			$maintain_datanip = array("i_peg_nip"=>$data['i_peg_nip']);
			$db->update('sdm.tm_anak',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_kejuruan_militer',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_golongan_pangkat',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_jabatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pendidikan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_kerabat',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pasangan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_kesehatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_lhkpn',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_jabatan_fungsional',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_kemampuan_bahasa',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_kepangkatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_luarnegeri',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_organisasi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pelatihan_fungsional',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pelatihan_lain',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pelatihan_penjenjangan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_pelatihan_teknis',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_penghargaan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_penilaian_ak',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_penilaian_dp3',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_penilaian_kinerjamil',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_potensi_akademik',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_sanksi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_seminar',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_sertifikasi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
			$db->update('sdm.tm_seminar',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nipb'])."'");
	...
	$maintain_data = array("i_peg_nip"=>$data['i_peg_nip'],
		...
		if ($par=='insert'){$db->insert('sdm.tm_pegawai',$maintain_data);}
		else if ($par=='update'){$db->update('sdm.tm_pegawai',$maintain_data, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");}	 
		if(trim($data['i_peg_nip']) != '' && trim($data['i_peg_nip']) != trim($data['i_peg_nip_old'])){
			//update table lain
			$maintain_datanip = array("i_peg_nip"=>$data['i_peg_nip']);
			$db->update('sdm.tm_anak',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_kejuruan_militer',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_golongan_pangkat',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_jabatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pendidikan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_kerabat',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pasangan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_kesehatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_lhkpn',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_jabatan_fungsional',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_kemampuan_bahasa',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_kepangkatan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_luarnegeri',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_organisasi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pelatihan_fungsional',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pelatihan_lain',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pelatihan_penjenjangan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_pelatihan_teknis',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_penghargaan',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_penilaian_ak',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_penilaian_dp3',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_penilaian_kinerjamil',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_potensi_akademik',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_sanksi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_seminar',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_sertifikasi',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
			$db->update('sdm.tm_seminar',$maintain_datanip, "i_peg_nip = '".trim($data['i_peg_nip_old'])."'");
		
		}
		//print $data['i_peg_nip_old']."<>";print_r($maintain_data);//exit;
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function maintainDataCuti(array $data,$par) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
	     $db->beginTransaction();
if($par=='update'){
 	...
 	public function getPegawaiListByNip($cari) 
	{
			...
			{		
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT e_file_photo,i_peg_nip,i_peg_nrp,i_peg_nip_new,n_peg,n_peg_gelardepan,n_peg_gelarblkg,c_peg_jeniskelamin,
					...
					{ 
						...
						$n_pangkat_cpns= $db->fetchOne("SELECT n_peg_pangkat FROM sdm.tr_golongan_pangkat WHERE c_peg_golongan ='$c_gol_cpns'  and c_peg_tipegolongan='$c_status_kepegawaian'");
...
public function getPegawaiList2($masukan, $pageNumber, $itemPerPage) 
	{
		...
		if(($pageNumber == 0) && ($itemPerPage == 0)){
			...
			$sql2 = "SELECT i_peg_nip,i_peg_nrp,i_peg_nip_new, n_peg,c_peg_status,c_status_kepegawaian,c_eselon,to_char(d_tmt_eselon,'dd-mm-yyyy') as d_tmt_eselon, c_jabatan,to_char(d_mulai_jabat,'dd-mm-yyyy') as d_mulai_jabat,c_peg_jeniskelamin, to_char(d_akhir_jabat,'dd-mm-yyyy') as d_akhir_jabat,c_golongan,(select n_peg_golongan from sdm.tr_golongan_pangkat where c_peg_golongan = a.c_golongan and c_peg_tipegolongan = a.c_status_kepegawaian) as n_golongan, (select n_peg_pangkat from sdm.tr_golongan_pangkat where c_peg_golongan = a.c_golongan and c_peg_tipegolongan = a.c_status_kepegawaian) as n_pangkat,d_tmt_golongan,v_gaji_pokok, c_lokasi_unitkerja,c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,c_gol_cpns, c_lokasi_unitkerja_cpns,c_eselon_cpns,e_file_photo,c_eselon_cpns,c_lokasi_unitkerja_cpns,c_eselon_i_cpns, c_eselon_ii_cpns,c_eselon_iii_cpns,c_eselon_iv_cpns,c_eselon_v_cpns,c_jabatan_cpns,to_char(d_tmt_kgb,'dd-mm-yyyy') as d_tmt_kgb, c_satker,c_parent,c_child,c_lokasi_unitkerja_pns,c_eselon_i_pns,c_eselon_ii_pns,c_eselon_iii_pns,c_eselon_iv_pns FROM sdm.tm_pegawai a $whereOpt  order by c_eselon asc limit $xLimit offset $xOffset";
			//echo $sql2;
...
public function updateMasaKerja(array $data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');
        try {
            $db->beginTransaction();
            ...
            $db->update('sdm.tm_pegawai',$dataMasaKerja, " i_peg_nip = '".trim($data['i_peg_nip'])."'");
            $db->commit();
            return 'sukses';
        } catch (Exception $e) {
            $db->rollBack();
...
public function getPegawaiNamaWithNip($cari) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');
        try {
            $db->setFetchMode(Zend_Db::FETCH_OBJ);
            $sql = "SELECT i_peg_nip,i_peg_nip_new,n_peg FROM sdm.tm_pegawai where $cari";
//            echo $sql;
//            exit;
            $result = $db->fetchAll($sql);
            $jmlResult = count($result);
