...
class sdm_refferensi_Service {
	...
	public function getGolonganPegawai($cari) 
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("select c_peg_golongan,n_peg_golongan,n_peg_pangkat,c_pph,c_peg_lvlgolongan,
													c_peg_tipegolongan
	...
	public function getPendidikan($cari) 
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					...
					FROM sdm.tr_pendidikan  where 1=1  $cari order by counter asc");
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("select c_jabatan,n_jabatan,e_keterangan,c_tkfgs,c_kelfgs,c_golr,
											...
											q_usia_pens,q_tktfgs,q_ak_minimal from sdm.tr_jabatan where 1=1 $cari order by c_jabatan asc");
					// echo "<br>select c_jabatan,n_jabatan,e_keterangan,c_tkfgs,c_kelfgs,c_golr,
											// c_golt,n_jenjang,c_tanda,c_eselon,c_strata,q_tunjangan,
											// q_usia_pens,q_tktfgs,q_ak_minimal from sdm.tr_jabatan where 1=1 $cari order by c_jabatan asc";					
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
					{$data[$j] = array("c_jabatan"=>(string)$result[$j]->c_jabatan,
							...
							//var_dump($result[$j]->c_kelfgs .'<>'.$result[$j]->q_tktfgs);
						}
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					...
					{ 		
						$data[$j] = array("c_lokasi_unitkerja"=>(string)$result[$j]->c_lokasi_unitkerja,
										...
										"n_unitkerja"=>trim((string)$result[$j]->n_unitkerja));
					}
	...
	public function getdataTrUnitKerja($cari) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$sql = "select distinct(c_bidang) as c_bidang,c_type,c_child from sdm.tr_unitkerja where 1=1 $cari";
		//echo "$sql<br>";
		//$result = $db->fetchAll("select c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,trim(n_unitkerja) as n_unitkerja,c_bidang,c_parent,c_child,c_satker from sdm.tr_unitkerja where 1=1 $cari");
		$result = $db->fetchAll($sql);
		$jmlResult = count($result);
		for ($j = 0; $j < $jmlResult; $j++) 
					{$data[$j] = array("c_bidang"=>(string)$result[$j]->c_bidang,
										"c_type"=>(string)$result[$j]->c_type,
										"c_child"=>(string)$result[$j]->c_child);
					}
	     return $data;;
	...
	/* public function getTrUnitKerjaPngl($cari) {
	   ...
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		//echo "select distinct(c_eselon_i) as c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,n_unitkerja,c_bidang,c_parent,c_child from sdm.tr_unitkerja where 1=1 $cari";
		$result = $db->fetchAll("select distinct(c_eselon_i) as c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,n_unitkerja,c_bidang,c_parent,c_child from sdm.tr_unitkerja where 1=1 $cari");
		$jmlResult = count($result);
	     return $result;;
	   } catch (Exception $e) {
         echo $e->getMessage().'<br>';
	...
	public function getTingkatFungsional($cari) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$result = $db->fetchAll("select c_tkfgs ,q_tktfgs, n_title  from sdm.tr_tingkat_fungsional where 1=1 $cari order by q_tktfgs asc  ");				 
		$jmlResult = count($result);
	     return $result;;
	   } catch (Exception $e) {
         echo $e->getMessage().'<br>';
	...
	public function getTrHukuman($cari) { 
	   ...
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$sql = "select c_hukuman ,n_hukuman  from sdm.tr_jenis_hukuman where 1=1 $cari order by c_hukuman asc";
		//echo $sql;
		$result = $db->fetchAll($sql);				 
		$jmlResult = count($result);
	     return $result;;
