...
class Sdm_Pegawai_Service {
 	...
 	public function getPegawaiList($cari,$currentPage, $numToDisplay,$orderby) 
	{
			...
			{
				$data = $db->fetchOne("select count(*) from  sdm.tm_pegawai where c_stat_aktivasi='A' $cari");
			...
			{	
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT i_peg_nip,i_peg_nrp, n_peg,c_peg_status,c_status_kepegawaian,c_eselon,to_char(d_tmt_eselon,'dd-mm-yyyy') as d_tmt_eselon,
								...
								FROM sdm.tm_pegawai where c_stat_aktivasi='A' $cari  $orderby   
	...
	public function maintainDataCpns(array $data,$par) {
	   ...
	   try {
	     ...
	     $maintain_data = array("i_peg_nip"=>$data['i_peg_nip'],
								...
								"d_tmt_kerja"=>$data['d_tmt_kerjacpns'],
			...
			{		
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT i_peg_nip,i_peg_nrp,i_peg_nip_new,n_peg,n_peg_gelardepan,n_peg_gelarblkg,c_peg_jeniskelamin,
										...
										d_spmt_cpns,i_spmt_cpns,n_spmt_pejabatcpns,i_peg_npwp,i_peg_rekening,c_peg_bank
