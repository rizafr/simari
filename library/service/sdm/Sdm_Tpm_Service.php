...
class Sdm_Tpm_Service {
 	...
 	public function getTpmUsulan($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$sql ="select usulan_id,usulan_nomor,usulan_keterangan,to_char(mod_date,'dd-mm-yyyy') as mod_date,
								tvinstansi_kd,periode_text,periode_keterangan,
								c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,
								c_parent,c_satker,c_eselon,
								ket_pratpm,ket_tpm1,ket_tpm2,
								to_char(tgl_pratpm,'dd-mm-yyyy') as tgl_pratpm,
								to_char(tgl_tpm1,'dd-mm-yyyy') as tgl_tpm1,
								to_char(tgl_tpm2,'dd-mm-yyyy') as tgl_tpm2,
								c_proses
					...
					{ 
						...
						$c_proses=trim($result[$j]->c_proses);
						$usulan_id=trim($result[$j]->usulan_id);
						//$tpm_status = $db->fetchOne(" SELECT tpm_status FROM sdm.tm_pegawai_tpm WHERE usulan_id='$usulan_id'");
	...
	public function maintainDataMaster(array $data,$par,$jtpm = null) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
	     $db->beginTransaction();
	    if($jtpm == '' || $jtpm == 'pratpm') {
	     $maintain_data = array("tgl_pratpm"=>$data['tgl_pratpm'],
							"ket_pratpm"=>$data['ket_pratpm'],
							"c_proses"=>$data['c_proses']
							);
		} else  if($jtpm == 'tpm1') {
			 $maintain_data = array("tgl_tpm1"=>$data['tgl_tpm1'],
							"ket_tpm1"=>$data['ket_tpm1'],
							"c_proses"=>$data['c_proses']
							);
		} else  if($jtpm == 'tpm2') {
			 $maintain_data = array("tgl_tpm2"=>$data['tgl_tpm2'],
							"ket_tpm2"=>$data['ket_tpm2'],
							"c_proses"=>$data['c_proses']
							);
		}
	...
	public function maintainDataTpmSK(array $data) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
	     $db->beginTransaction();
	     
	     $maintain_data = array("n_sk"=>$data['n_sk'],
				"d_sk"=>$data['d_sk'],
				"c_stat_cetak"=>$data['c_stat_cetak'],				
				"n_pejabat"=>$data['n_pejabat']);
		$db->update('sdm.tm_usulan_tpm',$maintain_data, "usulan_id = '".trim($data['usulan_id'])."'");
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
 ...
 //==================================================================================================================================
 	public function getTpmPegawai($cari) 
	{
			...
			try 
			{
				...
				to_char(tgl_tpm2,'dd-mm-yyyy') as tgl_tpm2,
				usulan_rekomendasi_pratpm,usulan_rekomendasi_tpmi,usulan_rekomendasi_tpmii,
				c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv
	...
	public function maintainDataTpmPegawaiUpdtTpmi(array $data,$par) {
	   ...
	   try {
		 ...
		 $ket_tpm1 = $data['nama_list'];
		 $tpm_status = 1;
		 $maintain_data = array("tpm_status"=>$tpm_status,
				"tpm_status_2"=>$data['tpm_status_2'],
				"tgl_tpm1"=>$tgl_tpm1,
				"ket_tpm1"=>$ket_tpm1,
				"c_proses"=>$data['cd_proses']);
	...
	public function UpdateStatusFinish(array $data) {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   try {
	     $db->beginTransaction();
		$tpm_status = 2;			
	     $maintain_data = array("tpm_status"=>$tpm_status);
		$db->update('sdm.tm_pegawai_tpm',$maintain_data, "usulan_id = '".trim($data['usulan_id'])."'");
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
...
public function getListTpm3($cari) 
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					...
					/*$sql = "select distinct(a.usulan_id) as usulan_id ,a.usulan_nomor,
								...
								where  b.id_list = c.id_list $cari";*/
							$sql = "select distinct(a.usulan_id) as usulan_id ,a.usulan_nomor,
								to_char(mod_date,'dd-mm-yyyy') as mod_date,b.usulan_id,b.id_list,
								a.tvinstansi_kd,a.periode_text,a.periode_keterangan,a.usulan_keterangan,
								peg_nip,tpm_namalengkap, tpm_status_1,  tpm_status_2,tpm_status_3,
								a.c_eselon_i,a.c_eselon_ii,a.c_eselon_iii,a.c_eselon_iv,a.c_parent,a.c_satker,
								n_sk,to_char(d_sk,'dd-mm-yyyy') as d_sk,c_stat_cetak,n_pejabat,c_eselon	
								from sdm.tm_usulan_tpm a, sdm.tm_pegawai_tpm b 
					...
					{ 
						...
						$data[$j] = array("usulan_id"=>(string)$result[$j]->usulan_id,
								...
								"c_stat_cetak"=>(string)$result[$j]->c_stat_cetak,
	...
	public function getListTpm5($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					/* $sql = "select distinct(a.usulan_id) as usulan_id, 
						ket_pratpm, to_char(tgl_pratpm,'dd-mm-yyyy') as tgl_pratpm, 
						ket_tpm1, to_char(tgl_tpm1,'dd-mm-yyyy') as tgl_tpm1, 
						ket_tpm2, to_char(tgl_tpm2,'dd-mm-yyyy') as tgl_tpm2, tpm_status	
						from sdm.tm_pegawai_tpm a  where  $cari"; */
					$sql = "select distinct(a.usulan_id) as usulan_id 	
						from sdm.tm_pegawai_tpm a  where  $cari";						
					//echo $sql;
					$result = $db->fetchall($sql);	
					...
					{ 
						$usulan_id=(string)$result[$j]->usulan_id;
						$usulan_nomor = $db->fetchOne("select usulan_nomor from sdm.tm_usulan_tpm where usulan_id='$usulan_id'");
						$ket_pratpm = $db->fetchOne("select ket_pratpm from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$tgl_pratpm = $db->fetchOne("select to_char(tgl_pratpm,'dd-mm-yyyy') as tgl_pratpm from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$ket_tpm1 = $db->fetchOne("select ket_tpm1 from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$tgl_tpm1 = $db->fetchOne("select to_char(tgl_tpm1,'dd-mm-yyyy') as tgl_tpm1 from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$ket_tpm2 = $db->fetchOne("select ket_tpm2 from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$tgl_tpm2 = $db->fetchOne("select to_char(tgl_tpm2,'dd-mm-yyyy') as tgl_tpm2 from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
						$tpm_status = $db->fetchOne("select tpm_status from sdm.tm_pegawai_tpm where usulan_id='$usulan_id'");
