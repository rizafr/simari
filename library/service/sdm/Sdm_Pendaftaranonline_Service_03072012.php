...
public function getJabatanUsulan() 
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("select b.n_surat,b.n_jabatan_usul FROM sdm.tm_cpns_usul a, sdm.tm_cpns_usuljabatan b
								where a.n_surat=b.n_surat and c_aktivasi='1'  order by n_jabatan_usul asc");		
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
					{ 

						$data[$j] = array("n_surat"=>$result[$j]->n_surat,"n_jabatan_usul"=>(string)$result[$j]->n_jabatan_usul);									
					}
      ...
      {    
        ...
        $db->setFetchMode(Zend_Db::FETCH_OBJ);      
          $result = $db->fetchAll("SELECT i_ktp,n_pendaftar,c_jeniskelamin,
                c_agama,c_golongan_darah,c_statusnikah,n_hobi,to_char(d_lahir,'dd-mm-yyyy') as d_lahir,c_propinsi_lahir,
                ...
                n_pend_lembaga,c_pend_akreditasi,c_posisi_jabatan,c_wil_pengadilan,e_file_photo,n_register,n_surat_usuljabat
                FROM sdm.tm_daftaronline where 1=1 $cari");
          $jmlResult = count($result);
          for ($j = 0; $j < $jmlResult; $j++) 
          { 
