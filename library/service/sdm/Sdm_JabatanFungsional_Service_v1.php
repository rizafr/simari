...
class Sdm_JabatanFungsional_Service {
 	...
 	public function getJabatanList($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT  i_peg_nip,c_statusjabatan,c_alasan,c_eselon,c_jabatan,
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
		...
		if ($par=='update'){$db->update('sdm.tm_jabatan_fungsional',$maintain_data, "i_peg_nip = '".trim($data['i_peg_nip'])."'  and c_statusjabatan = '".trim($data['c_statusjabatan2'])."' and c_alasan = '".trim($data['c_alasan2'])."' and to_char(d_mulai_jabat,'dd-mm-yyyy') = '".trim($data['d_mulai_jabat2'])."'");}	 
		if ($par=='delete'){$db->delete('sdm.tm_jabatan_fungsional', "i_peg_nip = '".trim($data['i_peg_nip'])."'  and c_statusjabatan = '".trim($data['c_statusjabatan'])."' and c_alasan = '".trim($data['c_alasan'])."' and to_char(d_mulai_jabat,'dd-mm-yyyy') = '".trim($data['d_mulai_jabat'])."' ");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
