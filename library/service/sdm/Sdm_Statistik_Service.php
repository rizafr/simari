...
class Sdm_Statistik_Service {
	...
	public function getStatistikData($cari) 
	{
	...
	public function getCountData($cari) 
	{
			$registry = Zend_Registry::getInstance();
			$db = $registry->get('db');
			try 
	...
	public function getLokasi($cari) {
	   ...
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$result = $db->fetchOne("select n_lokasi  from sdm.tr_lokasi where 1=1 $cari ");				 		
	     return $result;;
	   } catch (Exception $e) {
         echo $e->getMessage().'<br>';
	     return 'gagal';
			...
			{		
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT i_peg_nip,i_peg_nip_new,n_peg,n_peg_gelardepan,n_peg_gelarblkg,c_peg_jeniskelamin,
										...
										FROM sdm.tm_pegawai where c_stat_aktivasi='A' $cari");
...
public function getCountDataForEis($cari) 
	{
			$registry = Zend_Registry::getInstance();
			$db = $registry->get('db');
			try 
			{		
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll(" select c_eselon_i,count(i_peg_nip) as jumlah from sdm.tm_pegawai where c_eselon_i in ('01','03','04','05','06','07') $cari group by c_eselon_i order by c_eselon_i asc");
	
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
					{ 
						$data[$j] = array("c_eselon_i"=>(string)$result[$j]->c_eselon_i,"jumlah"=>(string)$result[$j]->jumlah);
					}
