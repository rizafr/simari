...
class sdm_refferensi_Service {
	...
	public function getTrUnitKerja($cari) {
	   ...
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$result = $db->fetchAll("select c_eselon_i,c_eselon_ii,c_eselon_iii,c_eselon_iv,c_eselon_v,n_unitkerja,c_bidang,c_parent,c_child,c_satker from sdm.tr_unitkerja where 1=1 $cari");
		$jmlResult = count($result);
	     return $result;;
	   } catch (Exception $e) {
         echo $e->getMessage().'<br>';
	...
	public function getTrUnitKerjaPngl($cari) 
	{
			...
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("select distinct(c_eselon_i) as c_eselon_i from sdm.tr_unitkerja where 1=1  and c_lokasi_unitkerja='3'");
