 	...
 	public function getPendidikanList($cari) 
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("SELECT  id,i_peg_nip,c_pend_jenis,c_pend,n_pend_lembaga,n_pend_jurusan,
										...
										e_pend_skripsi,e_keterangan,c_pend_status,i_entry,d_entry,c_akreditasi
										FROM sdm.tm_pendidikan where 1=1 $cari  order by d_pend_akhir desc");	
					...
					{ 
						...
						if($result[$j]->c_pend != ''){
							$namaPendidikan = $db->fetchOne("SELECT n_pend FROM sdm.tr_pendidikan WHERE c_pend = '".trim($result[$j]->c_pend)."'");
						}
						...
						"i_peg_nip"  =>(string)$result[$j]->i_peg_nip,
										...
										"c_akreditasi"=>(string)$result[$j]->c_akreditasi,										
										"n_pend"=>$namaPendidikan,
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
	     ...
	     $maintain_data = array("i_peg_nip"=>$data['i_peg_nip'],
							...
							"c_akreditasi"=>$data['c_akreditasi'],							
		...
		if ($par=='update'){$db->update('sdm.tm_pendidikan',$maintain_data, "id = '".trim($data['id'])."'");}	 
		if ($par=='delete'){$db->delete('sdm.tm_pendidikan', "id = '".trim($data['id'])."'");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
