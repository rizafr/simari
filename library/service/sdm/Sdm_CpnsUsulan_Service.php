   	...
   	public function getListCheckJabatCpns($cari) 
	{
			...
			try 
			{
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					...
					{ 
						$c_kualifikasi_pend=trim($result[$j]->c_kode);
						$n_kualifikasi_pend = $db->fetchOne("select n_kualifikasi_pend from sdm.tr_kulifikasi_pendidikan where c_kualifikasi_pend='$c_kualifikasi_pend'");

						$data[$j] = array("y_tahun"=>(string)$result[$j]->y_tahun,
								"c_jabatan"=>$result[$j]->c_jabatan,
								"n_jabatan"=>(string)$result[$j]->n_jabatan,
								"c_kode"=>(string)$result[$j]->c_kode,
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
					$result = $db->fetchAll("select id,n_surat,to_char(d_surat,'dd-mm-yyyy') as d_surat,n_jabatan_usul,n_pend_usul,i_entry,d_entry,
								c_jabatan_usul,c_pend_usul,c_kualifikasi_pend
					...
					{ 
						...
						$data[$j] = array("id"=>(string)$result[$j]->id,
								...
								"c_jabatan_usul"=>(string)$result[$j]->c_jabatan_usul,
								"c_pend_usul"=>(string)$result[$j]->c_pend_usul,
								"c_kualifikasi_pend"=>(string)$result[$j]->c_kualifikasi_pend,
								"i_entry"=>(string)$result[$j]->i_entry,
								"d_entry"=>(string)$result[$j]->d_entry);									
					}
	...
	public function maintainDataUsulJabatan(array $data,$par) {
	   ...
	   try {
	     ...
	     $maintain_data = array("n_surat"=>$data['n_surat'],
				...
				"c_jabatan_usul"=>$data['c_jabatan_usul'],
				"c_pend_usul"=>$data['c_pend_usul'],
				"c_kualifikasi_pend"=>$data['c_kualifikasi_pend'],
				"i_entry"=>$data['i_entry'],
				"d_entry"=>date("Y-m-d"));
		...
		if ($par=='delete'){$db->delete('sdm.tm_cpns_usuljabatan', "n_surat = '".trim($data['n_surat'])."' and to_char(d_surat,'dd-mm-yyyy') = '".$data['d_surat']."'");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
