...
class Cms_kategoriprodhukum_Service {
	...
	public function getkategoriprodukhukumPubList($cari)
	{
	   ...
	   try {
		$xLimit=10;
		 ...
		 $strQuery="SELECT c_kategori,n_judul,c_status,i_entry,d_entry  from portal.trkategoriprodukhukum where c_status=1 $cari order by n_judul ";
         //echo $strQuery;
 	...
 	public function getkategoriprodhukumList($cari,$currentPage, $numToDisplay,$orderby) 
	{
			...
			{$data = $db->fetchOne("select count(*) from  portal.trkategoriprodukhukum where 1=1 $cari");}
			else		
			{	
				$xLimit=$numToDisplay;
				$xOffset=($currentPage-1)*$numToDisplay;				
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
				    $strQuery="SELECT c_kategori,n_judul,c_status,i_entry,d_entry  from portal.trkategoriprodukhukum where 1=1 $cari $orderby limit $xLimit offset $xOffset";
					$result = $db->fetchAll("$strQuery");
					$jmlResult = count($result);
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
		...
		if ($par=='insert'){$db->insert('portal.trkategoriprodukhukum',$maintain_data);}
		if ($par=='update'){$db->update('portal.trkategoriprodukhukum',$maintain_data, "c_kategori = '".trim($data['c_kategori'])."' ");}	 
		if ($par=='delete'){$db->delete('portal.trkategoriprodukhukum', "c_kategori = '".trim($data['c_kategori'])."'  and i_entry = '".trim($data['i_entry'])."' ");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
