...
class Cms_produkhukum_Service {
			...
			{	
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
				    $strQuery="SELECT c_produkhukum,c_kategori,n_judul,n_detil,n_file,c_status,i_entry,d_entry,i_nomor_produkhukum,d_tahun_produkhukum
								from portal.tmprodukhukum where 1=1 $cari $orderby limit $xLimit offset $xOffset";
					//echo $strQuery;			
					...
					{ 
						...
						$strQueryx="SELECT n_judul FROM portal.trkategoriprodukhukum WHERE c_kategori ='$c_kategori'";
						//echo $strQueryx;
 	...
 	public function getprodukhukumTenPubList($cari) 
	{
			...
			try 
			{
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
				    $strQuery="SELECT c_produkhukum,c_kategori,n_judul,n_detil,n_file,c_status,i_entry,d_entry,i_nomor_produkhukum,d_tahun_produkhukum
								from portal.tmprodukhukum where c_status='1' $cari order by d_tahun_produkhukum limit $xLimit offset $xOffset";
					//echo $strQuery;			
					$result = $db->fetchAll("$strQuery");
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
 	...
 	public function getprodukhukumPubList($cari,$currentPage, $numToDisplay,$orderby) 
	{
			...
			{	
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
				    $strQuery="SELECT c_produkhukum,c_kategori,n_judul,n_detil,n_file,c_status,i_entry,d_entry,i_nomor_produkhukum,d_tahun_produkhukum
								from portal.tmprodukhukum where c_status='1' $cari $orderby limit $xLimit offset $xOffset";
					//echo $strQuery;			
					$result = $db->fetchAll("$strQuery");
					$jmlResult = count($result);
					for ($j = 0; $j < $jmlResult; $j++) 
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
		...
		if ($par=='insert'){$db->insert('portal.tmprodukhukum',$maintain_data);}
		if ($par=='update'){$db->update('portal.tmprodukhukum',$maintain_data, "c_produkhukum = '".trim($data['c_produkhukum'])."' ");}	 
		if ($par=='delete'){$db->delete('portal.tmprodukhukum', "c_produkhukum = '".trim($data['c_produkhukum'])."'  and i_entry = '".trim($data['i_entry'])."' ");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function getKategoriHukum() {
	   ...
	   try {
		$db->setFetchMode(Zend_Db::FETCH_OBJ);
		$result = $db->fetchAll("SELECT c_kategori,n_judul from portal.trkategoriprodukhukum where c_status=1");						 
		$jmlResult = count($result);
	     return $result;;
	   } catch (Exception $e) {
         echo $e->getMessage().'<br>';
