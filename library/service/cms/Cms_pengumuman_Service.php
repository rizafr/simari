...
class Cms_pengumuman_Service {
    ...
    public function findpengumumanByKey($id) {
	   ...
	   try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
         $result = $db->fetchRow("SELECT c_pengumuman,n_judul,n_detil,d_pengumuman,c_status  from portal.tmpengumuman where c_pengumuman='$id' ");
		 //echo "SELECT c_pengumuman,n_judul,n_detil,d_pengumuman,c_status  from portal.tmpengumuman where c_pengumuman='$id' ";
	     $output = array("c_pengumuman"=>(string)$result->c_pengumuman,
		                       "n_judul"=>(string)$result->n_judul,
		                       "n_detil"=>(string)$result->n_detil,
		                       "d_pengumuman"=>(string)$result->d_pengumuman,
