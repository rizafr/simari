...
class Cms_Berita_Service {
	...
	public function getBeritaPubList($cari)
	{
	   ...
	   try {
		 ...
		 $strQuery="SELECT c_berita,n_judul,n_detil,d_berita,n_sumber,c_status  from portal.tmberita where c_status=1 $cari order by d_berita desc";
         //echo $strQuery;
 	...
 	public function getBeritaList($cari,$currentPage, $numToDisplay,$orderby) 
	{
			...
			{
				$data = $db->fetchOne("select count(*) from  portal.tmberita  $cari");
			...
			{	
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);	
				    $strQuery="SELECT c_berita,n_judul,n_detil,d_berita,n_sumber,c_status  from portal.tmberita $cari $orderby limit $xLimit offset $xOffset";
					$result = $db->fetchAll("$strQuery");
    ...
    public function findBeritaByKey($id) {
	   ...
	   try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
         $result = $db->fetchAll("SELECT c_berita,n_judul,n_detil,d_berita,n_sumber,c_status  from portal.tmberita where c_berita='$id' ");
	...
	public function maintainData(array $data,$par) {
	   ...
	   try {
		...
		if ($par=='insert'){$db->insert('portal.tmberita',$maintain_data);}
		if ($par=='update'){$db->update('portal.tmberita',$maintain_data, "c_berita = '".trim($data['c_berita'])."' ");}	 
		if ($par=='delete'){$db->delete('portal.tmberita', "c_berita = '".trim($data['c_berita'])."'  and i_entri = '".trim($data['i_entri'])."' ");}
		$db->commit();
	     return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function maintainHapusData($idberita,$userlogin) {
	   ...
	   try {
		...
		$db->fetchOne("delete from  portal.tmberita  where c_berita='$idberita' and i_entri='$userlogin'");
		$db->commit();
		return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
 	...
 	public function getMaxId() 
	{
			...
			{		
				...
				$db->setFetchMode(Zend_Db::FETCH_OBJ);			
				$data = $db->fetchOne("select count(*) from  portal.tmberita ");
				return $data;
			} catch (Exception $e) 
			{
		         	echo $e->getMessage().'<br>';
