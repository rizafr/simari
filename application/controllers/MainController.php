...
class MainController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
		...
		//====================
		//echo 'noel: '.$this->adminuser_serv->checkLogin($dataMasukan);
		...
		//echo "test $testldap";
		//echo 'noel1: '.$this->view->checklogin;
		if($this->view->checklogin){
			$this->_helper->layout->setLayout('cms-layout');	
			...
			if(count($this->view->UserWewenang)> 0){
				for($x=0; $x<count($this->view->UserWewenang); $x++){
					...
					$userIzin[0]	= array('c_izin' => '000002',
											'n_izin' => 'PENGELOLA_DALAM_SEKTORAL');
			...
			*/
			...
			foreach ($arraycheckotoritas as $key => $val){
			 ...
			 //echo $arrayc_sektoral['4'].'<>'.$arrayc_sektoral['1'].'<br>';
			 //echo $arrayc_wewenang['4'].'<>'.$arrayc_wewenang['1'];
			 //exit;
			...
			if(count($this->view->checkotoritas)>0){
			$ssologin->checkotoritas		= $this->view->checkotoritas; 
			}
			if($this->view->UserWewenang[0]['c_wewenang']){
				//$ssologin->wewenang		= $this->view->UserWewenang[0]['c_wewenang'];
			} else { // ambil status default
				//$ssologin->wewenang	= 'O'; //Operator
			}
			if($this->view->UserWewenang[0]['c_sektoral']){
				//$ssologin->sektoral		= $this->view->UserWewenang[0]['c_sektoral'];
			} else { // ambil status default
				//$ssologin->sektoral = 'D'; //Dalam sektoral
			}
