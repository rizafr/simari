...
class Sdmmodule_DataCpnsController extends Zend_Controller_Action {
    ...
    public function init() {
		...
		if ($this->view->c_izin=='000002' || $this->view->c_izin=='000003'){
			$this->view->jdl="Kelola ";
	...
	if ($c_status_kepegawaian=='2' || $c_status_kepegawaian=='3')
...
public function cpnsAction() {
	...
	if ($this->view->c_izin=='000002' || $this->view->c_izin=='000003'){
		...
		$c_pend_cpns = $this->view->c_pend_cpns;
		$nmJenjangList = $this->reff_serv->getPendidikan(" and c_pend='$c_pend_cpns'");	
		$this->view->n_pend_cpns = $nmJenjangList[0]['n_pend'];	
		$c_status_kepegawaian 	= $this->view->c_status_kepegawaian;
		$c_gol_cpns				= $this->view->c_gol_cpns;
		
		$carigol=" and c_peg_golongan='$c_gol_cpns' and c_peg_tipegolongan ='$c_status_kepegawaian' ";	
		$statusGolRef = $this->reff_serv->getGolonganPegawai($carigol); 
		$this->view->n_peg_pangkat = $statusGolRef[0]['n_peg_pangkat'];
		$caristat =" and c_peg_status='$c_status_kepegawaian' ";
		$statusKePegRef = $this->reff_serv->getStatusPegawai($caristat); 
		$this->view->n_peg_status = $statusKePegRef[0]['n_peg_status'];
		...
		$this->view->jdl="Melihat ";$this->render('cpnsview');
	}	
...
public function maintaindataAction() {
	...
	$i_peg_nip_old = $_POST['i_peg_nip_old'];
	
	$cari = " and (i_peg_nip ='$i_peg_nip' or i_peg_nip_new ='$i_peg_nip') ";
	$datapegawai=$this->pegawai_serv->getPegawaiListByNip($cari );
	$cekdata=count($datapegawai);
	if (($cekdata==1) && ($_POST['proses']=='Simpan' || ($_POST['proses'] !='Simpan' && $i_peg_nip != $i_peg_nip_old))) {
	...
	else
	{
		...
		{		
			...
			$c_eselon_ii=$_POST['c_eselon_ii_cpns'];
			if ($_POST['c_eselon_ii_cpns']!=''){$c_eselon_iil=strlen($c_eselon_ii); $c_eselon_ii=$this->right2($c_eselon_ii, $c_eselon_iil);}
...
//{
	...
	if (!$_POST['i_peg_nip_new']){$i_peg_nip_new='';}
	...
	if ($_POST['proses']=='Simpan')
	{
		$MaintainData = array("i_peg_nip"=>$i_peg_nip,
						...
						"c_lokasi_unitkerja"=>$_POST['c_lokasi_unitkerja_cpns'],
						...
						"c_parent"=>$c_parent,
						"c_satker"=>$c_satker,
						"c_eselon"=>$_POST['c_eselon_cpns'],
						"c_jabatan"=>$_POST['c_jabatan_cpns'],
						"c_golongan"=>$c_gol_cpns,
						...
						"c_parent_cpns"=>$c_parent,
						"c_satker_cpns"=>$c_satker,
						"c_peg_status"=>'2CP',
						"e_file_photo"=>'nophoto.jpg',		
						"i_entry"=>$this->view->userid,
						"d_entry"=>date('Ymd'));
						$arraydata = array("c_peg_status"=>'2CP',
							"e_file_photo"=>'nophoto.jpg'
							);
		//array_push($arraydata, $MaintainData);				
	...
	{
		if($_POST['i_peg_nip_old'] != $_POST['i_peg_nip']){
			if($_POST['e_file_photo'] != '' && $_POST['e_file_photo'] != 'nophoto.jpg'){
				$e_file_photo_old = $_POST['e_file_photo'];
				list($nmfile,$extfoto) = split('[.]',$e_file_photo_old);
				$extfoto = substr($e_file_photo_old, strrpos($e_file_photo_old, '.') + 1);
				$e_file_photo = $i_peg_nip.'.'.$extfoto;
				$fotolama = '../library/data/sdm/photo/'.$e_file_photo_old;
				$fotobaru = '../library/data/sdm/photo/'.$e_file_photo;
				$gantifoto = true;
			}  else {
				$e_file_photo = 'nophoto.jpg';
				$gantifoto = '';
			...
			$e_file_photo = $_POST['e_file_photo'];
		}
		$MaintainData = array("i_peg_nip"=>$i_peg_nip,
						"i_peg_nip_old"=>$_POST['i_peg_nip_old'],
						"i_peg_nrp"=>$_POST['i_peg_nrp'],
		...
		if($hasil == 'sukses' && $gantifoto != ''){
			if (file_exists("$fotobaru")) unlink($fotobaru);
...
public function listDataByKey($nip) {  
	...
	$this->view->i_peg_nip_old=$datapegawai[0]['i_peg_nip'];	
...
else
{
	...
	$this->view->c_eselon_i_cpns=trim($datapegawai[0]['c_eselon_i']).";".trim($datapegawai[0]['neselon1']);
	$this->view->c_eselon_ii_cpns=trim($datapegawai[0]['c_eselon_ii']).";".trim($datapegawai[0]['c_parent']).";".trim($datapegawai[0]['neselon2']);
	$this->view->c_eselon_iii_cpns=trim($datapegawai[0]['ceselon2']).";".trim($datapegawai[0]['c_eselon_iii']).";".trim($datapegawai[0]['c_satker']).";".trim($datapegawai[0]['neselon3']);
	$this->view->c_eselon_iv_cpns=trim($datapegawai[0]['c_eselon_iv']).";".trim($datapegawai[0]['neselon4']);	
