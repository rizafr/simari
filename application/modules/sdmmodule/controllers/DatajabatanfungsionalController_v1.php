...
public function jabatanAction() {
	...
	if ($par=='insert'){
		...
		$this->listDataByKey($this->view->nip,$_GET['c_statusjabatan'],$_GET['c_alasan'],$_GET['d_mulai_jabat']);
	}
	else{
		$this->view->par="Ubah";
...
if (($cektglmulai==true &&  $cektglakhir==true && $cektglsk==true) )
{
	...
	$q_tktfgs=$datajab[0]['q_tktfgs'];

		if ($_POST['a_file'])
		{
				$dsk=$d_sk_jabat3.$d_sk_jabat2.$d_sk_jabat1;	
				$namefile=trim($_POST['i_peg_nip'])."_".$_POST['c_jabatan']."_".$dsk;
...
public function hapusdataAction() {
	...
	$MaintainData = array("i_peg_nip"=>($this->view->nip),
				"c_statusjabatan"=>$_GET['c_statusjabatan'],
				"c_alasan"=>$_GET['c_alasan'],
				"d_mulai_jabat"=>$_GET['d_mulai_jabat']);		
	$hasil = $this->jabatan_serv->maintainData($MaintainData,'delete');

	if ($hasil=='sukses')
	{
		$this->updateTmPegawai($this->view->nip);
	}	
...
public function listDataByKey($nip,$c_statusjabatan,$c_alasan,$d_mulai_jabat) {  
	$carilist = " and i_peg_nip='$nip' and c_statusjabatan='$c_statusjabatan' and c_alasan='$c_alasan' and to_char(d_mulai_jabat,'dd-mm-yyyy')='$d_mulai_jabat'";
