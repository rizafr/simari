...
public function listformulir2Action() {
	...
	if($proses == 'prs') $this->view->listGol = $this->formulir2_serv->getdatagol();
	else if($proses == 'upd') $this->view->listGol = $this->formulir2_serv->getdataformasi($tahun);
	
}

public function simpanformulirAction(){
	$listGol = $this->formulir2_serv->getdatagol();
	$tahun	= $_POST["tahun"];
	$this->view->tahun=$tahun;
	$proses	= $_POST["proses"];
		$this->formulir2_serv->hapusformulir($tahun);
	...
	foreach ($listGol as $datapkt){
		...
		$masukanInsert = array("c_golongan" => $c_peg_golongan,
			...
			$hasil = $this->formulir2_serv->tambahformulir($masukanInsert);
