...
class Sdmmodule_DatapegawaiController extends Zend_Controller_Action {
	...
	public function init() {
		...
		if($this->view->wewenang == 'O'){$this->view->c_izin='000002';}
		 ...
		 //echo "noer:".$this->view->wewenang;exit;
		$this->view->c_eselon_i=$ssologin->c_eselon_i;
...
/* 	if ($c_eselon_i!='01'  ){
	...
	//if ($cCol=='i_peg_nip' && $_POST['nCol']!='17'){$cari .= " and (i_peg_nip like '%$nCol%' or  i_peg_nip_new like '%$nCol%')";}else{	$cari .= " and upper($cCol) like '%$nCol%' ";}
		...
		if ($cCol=='i_peg_nip'){$cari .= " and (i_peg_nip like '%$nCol%' or  i_peg_nip_new like '%$nCol%')";}else{
...
if($this->view->sektoral=='S'){// semua sektor
   	$c_lokasi_unitkerja=trim($this->view->c_lokasi_unitkerja);
	$c_eselon_i=trim($this->view->c_eselon_i);
	if ($c_eselon_i!='01'){
...
} else if ($this->view->c_eselon=='15' && $this->view->wewenang=='E'){//eksektuif dan staf
...
if ($_POST['i_peg_nip_new']){	
	...
	else
	{
		...
		if ($cekdata==1 && ($_POST['i_peg_nip'] != $i_peg_nip_old)) {
			...
			if ($_POST['i_peg_nip']){
				$i_peg_nip=$_POST['i_peg_nip'];}
			else{$i_peg_nip=$_POST['i_peg_nip_new'];}
...
$this->view->c_eselon_i =$_GET['eseloni'];
	...
	if ($_GET['c_lokasi_unitkerja']=='3'){ //lingkup pengadilan
	...
	if ($this->view->sektoral=='D'){
		...
		if ($_GET['c_lokasi_unitkerja']=='3'){
			...
			$this->view->eselonivList = $this->reff_serv->getTrUnitKerja(" and c_eselon_i='$c_eselon_i'  and c_eselon_ii='$c_eselon_ii'  and c_eselon_iii='$c_eselon_iii' and c_eselon_iv='$c_eselon_iv' and trim(c_tkt_esl)='4' and c_lokasi_unitkerja='$c_lokasi_unitkerja'");
	...
	if ($this->view->sektoral=='L'){
		...
		$expesl3 = explode(";",$c_eselon_iii);	
		$c_eselon_iii=$expesl3[0];
		if($c_lokasi_unitkerja ==1) $this->view->eselonivList = $this->reff_serv->getTrUnitKerja(" and c_eselon_i='$c_eselon_i'  and c_eselon_ii='$c_eselon_ii'  and c_eselon_iii='$c_eselon_iii' and trim(c_tkt_esl)='4' and c_lokasi_unitkerja='$c_lokasi_unitkerja'");
		if($c_lokasi_unitkerja ==3) $this->view->eselonivList = $this->reff_serv->getTrUnitKerja(" and c_eselon_i='$c_eselon_i'  and c_eselon_ii='$c_eselon_ii'  and c_eselon_iii='$c_eselon_iii' and trim(c_tkt_esl)='5' and c_lokasi_unitkerja='$c_lokasi_unitkerja'");
...
/* 	if ($c_eselon_i!='01'  ){
	...
	if ($_POST['cCol'])
	{
		$nCol=strtoupper(trim($_POST['nCol']));
		$cCol=$_POST['cCol'];		
		 $nCol= str_replace("\'", "''", $nCol);
		 
		 $nCola=trim($_POST['nCol']);
	...
	else
	{
		...
		$cCol=trim($_GET['cCol']);		
		 ...
		 $nCola=trim($_GET['nCol']);
		 $nCola= str_replace("\'", "'", $nCola);
		 $this->view->nCol = $nCola;
		 
		$this->view->cCol = trim($_GET['cCol']);	
	}
...
if($this->view->sektoral=='S'){ // Semua Sektoral
   	$c_lokasi_unitkerja=trim($this->view->c_lokasi_unitkerja);
	$c_eselon_i=trim($this->view->c_eselon_i);
	// if ($c_eselon_i!='01'){
		// $cari= " and (c_lokasi_unitkerja='$c_lokasi_unitkerja' or c_lokasi_unitkerja_cpns ='$c_lokasi_unitkerja') and (c_eselon_i='$c_eselon_i' or c_eselon_i_cpns='$c_eselon_i') ";
...
$cari= " and (i_peg_nip = '$i_peg_nip' or  i_peg_nip_new = '$i_peg_nip_new')";
	...
	if ($_GET['orderBy']){$orderBy=" order by $orderBy $order";}
	else{$orderBy=" order by c_golongan ASC,d_tmt_golongan ASC,c_eselon ASC,q_tktfgs,d_tmt_eselon ASC,d_tmt_cpns asc, q_tahun ASC,c_pend ASC,d_pend_akhir ASC,d_peg_lahir ASC";}
	$this->view->orderBy=$_GET['orderBy'];
	$currentPage=$_GET['currentPage'];
	if((!$currentPage) || ($currentPage == 'undefined'))
		{$currentPage = 1;}
