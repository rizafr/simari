...
public function cetakviewAction(){
		...
		$judul_lap= stripslashes($_POST['judul_lap']);
	 ...
	 if($s_nama)
	 {
		...
		if($_POST['sk_cpns']){$cari= $cari." and i_sk_cpns ='$sk_cpns'";}
		if($_POST['bln_pns']){$cari= $cari." and to_char(d_sk_pns,'m') ='$bln_pns'";}
		if($_POST['y_pns']){$cari= $cari." and to_char(d_sk_pns,'yyyy') ='$y_pns'";}
		if($_POST['y_lulus']){$cari= $cari." and d_pend_akhir ='$y_lulus'";}
		if($_POST['n_jurusan']){$cari= $cari." and lower(n_pend_jurusan) like '%".strtolower($n_jurusan)."%'";}
		if($_POST['n_universitas']){$cari= $cari." and lower(n_pend_lembaga) like '%".strtolower($n_universitas)."%'";}
		...
		if($_POST['y_diklatpim']){$cari= $cari." and q_tahun ='".$_POST['y_diklatpim']."'";}
		//if($_POST['propinsi_unit']){$cari= $cari." and c_penjenjangan =";} === blm ada di tabel
		//if($_POST['kd_jbt2']){$cari= $cari." and c_jabatan ='$kd_jbt2'";}
		...
		if($_POST['usia_awl'] && $_POST['usia_akh'] == ''){$cari= $cari." and EXTRACT(years from AGE(now(), d_peg_lahir)) = ".$_POST['usia_awl']."";}
		else if($_POST['usia_akh'] && $_POST['usia_akh'] != ''){$cari= $cari." and EXTRACT(years from AGE(now(), d_peg_lahir)) between ".$_POST['usia_awl']." and ".$_POST['usia_akh']."";}
		...
		//echo $cari;
		if($_POST['kd_tlp']){$cari= $cari." and i_peg_telponrumah ='$i_peg_telponrumah'";}
		if($_POST['kd_pos']){$cari= $cari." and a_peg_kodepos ='$kd_pos'";}
		if($_POST['w_kulit']){$w_kulit=strtoupper($w_kulit);$cari= $cari." and upper(n_peg_warnakulit) like '%$w_kulit%'";}
		if($_POST['kd_karpeg']){$cari= $cari." and i_peg_karpeg ='$kd_karpeg'";}
		if($_POST['kd_askes']){$cari= $cari." and i_peg_askes ='$kd_askes'";}
		if($_POST['kd_taspen']){$cari= $cari." and i_peg_taspen ='$kd_taspen'";}
		if($_POST['kd_korpri']){$cari= $cari." and i_peg_korpri ='$kd_korpri'";}
		if($_POST['kd_ktp']){$cari= $cari." and i_peg_ktp ='$kd_ktp'";}
		...
		$start=$_POST['start'];
		$limit=$_POST['limit'];	
		$orderby2_get	= $_POST['orderby2_get'];
		if(substr($orderby2_get,strlen($orderby2_get)-1,1) == ','){
			$orderby2_get = substr($orderby2_get,0,strlen($orderby2_get)-1);
		}		
		$this->view->orderby2_get=$orderby2_get;
		...
		if((!$currentPage) || ($currentPage == 'undefined'))
			...
			//$this->view->cari=$cari;
			$this->view->cari=addslashes($cari);

		if($orderby2_get != ''){ $orderBy =" order by ".$orderby2_get."";}
		
if ($_GET['par']=='exl')
{
	?>
	<script>
		var url = "<?php echo $this->basePath; ?>/sdmmodule/monitoringserbaguna/exl?find=<?echo $cari.($orderBy ? "&orderBy=$orderBy":"");?>";
		var wid='1000';
		var heg='600';
		var w = 0; 
		var h = 0;
...
$jdllap= $_POST['judul_lap'] ? stripslashes($_POST['judul_lap']) : '';		
...
<div style="height: 490px; overflow: auto; padding: 5px">';
 ...
 $html=$html.'						
	...
	$this->view->judul_lap=$_POST['judul_lap'] ? addslashes($_POST['judul_lap']) : '';
...
public function cetakviewhalAction(){
	 ...
	 if($s_nama)
	 {
		...
		if($_GET['sk_cpns']){$cari= $cari." and i_sk_cpns ='$sk_cpns'";}
		...
		if($_GET['y_diklatpim']){$cari= $cari." and q_tahun ='".$_GET['y_diklatpim']."'";} 
		...
		*/	
		...
		if($_GET['orderby2_get'] != ''){ $orderBy="order by ".$_GET['orderby2_get']."";}
		$totalpegawaiList = $this->pegawai_serv->getPegawaiList($cari, 0, 0 ,$orderBy);	
		$this->view->totalpegawaiList=$totalpegawaiList;		
		$pegawaiList = $this->pegawai_serv->getPegawaiList($cari,$currentPage,$numToDisplay,$orderBy );
...
$html2=$html2.'</tr><tr>';
		for ($j = 0; $j < count($pegawaiList); $j++) 
			{
				$no++;	
				 $no = (($currentPage -1)* $numToDisplay) +$j+1;
				if ($j%2==0) {
				      $html2=$html2.'<tr class="event">';
				 } else if ($j%2==1) { 
					  $html2=$html2.'<tr class="event2">';
...
// $this->view->judul_lap=addslashes($_GET['judul_lap']);
...
$this->view->view=$html;
$this->view->dt_tpl2_get=$dt_tpl2_get;
...
$cari=stripslashes($_GET['find']);
//$cari=$_GET['find'];
$orderby=$_GET['orderby'];
//$cari=str_replace('\\', "", $cari);

$html="";
//echo "noel: ".$cari , $orderby;
$pegawaiList = $this->pegawai_serv->getPegawaiListAll($cari,$orderby);
$totalpegawaiList =count($pegawaiList);
$jdllap=$_POST['judul_lap'];		
$html='<h2 class="title">'.$jdllap.'</h2>
<div style="height: 490px; overflow: auto; padding: 5px">';
 $html=$html.'
  <table align="center" border="1" cellspacing="1" cellpadding="2" class="sortable">
		<tr>
			<th width="5%" rowspan="2">No.</th>
			<th rowspan="2">Nama</th>
			<th rowspan="2">Jabatan</th>
