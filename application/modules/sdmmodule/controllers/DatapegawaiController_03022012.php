...
public function listpegawaiAction() {    
	...
	if ($_POST['cCol']=='unitkerja' || $_GET['cCol']=='unitkerja')
	{
		...
		else
		{
			...
			$this->view->c_lokasi_unitkerja=trim($_POST['c_lokasi_unitkerja']);
			$this->view->lokasiList = $this->reff_serv->getLokasi('');	
			$c_lokasi_unitkerja=trim($_POST['c_lokasi_unitkerja']);
			...
			$this->view->c_eselon_i =trim($_POST['c_eselon_i']);
			

			$c_eselon_ii=$_POST['eselonii'];
			$expesl2 = explode(";",$c_eselon_ii);
			$c_eselon_ii=$expesl2[0];
			$c_parent=$expesl2[1];
			$this->view->c_eselon_ii =trim($_POST['c_eselon_iii']);
			
			
			$c_eselon_iii=$_POST['eseloniii'];
			if ($c_eselon_iii){
			$expesl3 = explode(";",$c_eselon_iii);	
			$c_eselon_iii=$expesl3[0];
			$this->view->c_eselon_iii =trim($_POST['c_eselon_iii']);	
			}
			
			if ($c_eselon_i){$cari .= " and c_eselon_i='$c_eselon_i' and c_lokasi_unitkerja='$c_lokasi_unitkerja'";}
...
if ($_POST['d_peg_lahir']){
	$d_peg_lahir= substr($_POST['d_peg_lahir'],6,4)."-".substr($_POST['d_peg_lahir'],3,2)."-".substr($_POST['d_peg_lahir'],0,2);
			...
			if (($_POST['hrLhir3'] == ' ') || ($_POST['hrLhir3'] == '#')){
				$d_peg_lahir = null;
			}		
