...
jQuery(document).ready( function($) {
	...
	if ( ( 'undefined' != typeof wpAjax ) && $.isFunction( wpAjax.validateForm ) ) {
		jQuery('form.validate').submit( function() { return wpAjax.validateForm( $(this) ); } );
	}
...
adminMenu = {
	...
	fold : function(off) {
		if (off) {
			jQuery('#content').removeClass('folded');
			jQuery('#adminmenu li.wp-has-submenu').unbind();
		} else {
			jQuery('#content').addClass('folded');
