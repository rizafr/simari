...
//		$this->user_id	= $_REQUEST[user_id];
	...
	// method untuk melakukan koneksi ke server ldap
	function koneksiLdap(){
		$cfg		= $this->cfg;
		$ldapconn 	= ldap_connect($cfg['ldap_host']) or die("Could not connect to " . $cfg['ldap_host']);
		$root 		= $ldap_manager_user;
		$rootPwd 	= $ldap_manager_pwd;
 ...
 **/
	function searchUser($keyword, $offset = 0, $limit = 20){
		$cfg = $this->cfg;
		$ldapconn = $this->koneksiLdap();
		// list user yang ada di server ldap
...
//		$pid=$_REQUEST[pid];
		// filter & keyword
		// search au dan as berdasarkan username
		$search ="(&(cn=*$keyword*)(employeetype=$level)(businessCategory=$this->pid))";
		...
		{
			$content .= "<list>";
			$content .= "<username>".$info[$i]['cn'][0]."</username>\n";
			$content .= "</list>";					
		}
 ...
 */
	function addUser($username, $password, $level, $portal, $bumn_id, $emp_id, $email, $phone, $dp_no, $fullname){	
		// konfigurasi ldap
		$cfg 		= $this->cfg;
		$ldapconn 	= $this->koneksiLdap();
		...
		$info["uid"] 		= $username;
		$info["sn"] 		= $fullname?$fullname:'Nama Lengkap'; /* daniel 10/22/2008 */
...
//		$info["userpassword"] 	= "{CRYPT}".$pwd_md5;
		...
		$info["ou"] = $bumn_id;
		$portal=='oa'?$info["employeeNumber"] = $emp_id:"";
		$info["mail"]		= $email;
		$info["telephonenumber"]= $phone;
		if($dp_no)
		{
			$info["departmentnumber"]= $dp_no;
		}
