jQuery(document).ready(function () { 
	...
	if (loaderParm == null) {
		//location.href="#top";
	  var loader = jQuery('<div id="loading"><div class="image"><img alt="Loading..." src="../../images/ajaxLoading.gif"/></div><div class="load">Loading ...</div></div>')
...
function windowOpens(url){var w=0;var h=0;w=screen.availWidth;h=screen.availHeight;var popW=800,popH=500;var leftc=(w-popW)/2;var topc=(h-popH)/2;var selectWindow=window.open(url,'Selection','left='+leftc+',top='+topc+', width='+popW+',height='+popH+',resizable=0,scrollbars=yes')}function doCounter(countdown){document.getElementById("confirm").style.display="block";if(countdown>0){countdown=countdown-1;window.status=countdown+" seconds left to view this page.";setTimeout('doCounter()',5000)}else{document.getElementById("confirm").style.display="none"}}function orderBy(orderBy,order,url){var param={orderBy:orderBy,order:order};jQuery.get(url,param,function(data){jQuery("#tableview").html(data)})}function sdmValidasiData(xstr,FocusField,Message){alert(Message);eval(xstr+"."+FocusField+".focus();");return false;}
